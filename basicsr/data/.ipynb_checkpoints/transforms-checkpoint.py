...
def paired_random_crop(img_gts, img_lqs, gt_patch_size, scale, gt_path):
    ...
    if not isinstance(img_gts, list):
        img_gts = [img_gts.numpy()] #When using a torch
        #img_gts = [img_gts] 
    if not isinstance(img_lqs, list):
        #img_lqs = [img_lqs]
        img_lqs = [img_lqs.numpy()] #When using a torch

    print('LQ IMAGE SHAPE: ', img_lqs[0].shape)
    test, h_lq, w_lq = img_lqs[0].shape  #(5, 512, 512)
    test, h_gt, w_gt = img_gts[0].shape  #(5, 512, 512)
    lq_patch_size = gt_patch_size // scale
    ...
    if h_gt != h_lq * scale or w_gt != w_lq * scale:
        raise ValueError(
            ...
            f'multiplication of LQ ({h_lq}, {w_lq}).')
    if h_lq < lq_patch_size or w_lq < lq_patch_size:
         raise ValueError(f'LQ ({h_lq}, {w_lq}, {test}) is smaller than patch size '
                          f'({lq_patch_size}, {lq_patch_size}). '
                          f'Please remove {gt_path}.')
